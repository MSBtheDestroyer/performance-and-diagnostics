...
function get_wordpress_core_update_info() {
    ...
    if (isset($updates[0]->response) && $updates[0]->response == 'latest') {
        return (object)[
            'message' => 'WordPress Core is at the latest version!'
        ];
    } else {
        return array_map(function($update) {
            return (object)[
                'Available Core Version' => $update->current,
                'Recommended Action' => $update->response
            ];
        }, $updates);
    }
}
...
function get_wordpress_core_plugins_themes_info() {
    ...
    if (is_object($core_updates) && isset($core_updates->message)) {
        $sections[] = (object)[
            'data' => [(object)['WordPress Core Status' => $core_updates->message]]
        ];
    } else {
        $sections[] = (object)[
            'data' => $core_updates
    ...
    if (empty($theme_updates)) {
        $sections[] = (object)[
            'data' => [(object)['Active Theme Status' => 'Active theme is up to date! Good work!']]
        ];
    } else {
        $sections[] = (object)[
            'data' => $theme_updates
    ...
    if (empty($plugin_updates)) {
        $sections[] = (object)[
            'data' => [(object)['Active Plugin Status' => 'Active plugins are up to date! Good work!']]
        ];
    } else {
        $sections[] = (object)[
